...
Based on the exploration results, we determined applying downsampling technique with 4 different minority/majority label ratios (1:10, 1:4, 1:2 and 1:1), we would select Logistic Regression, Random Forest, Xgboost and LightGBM with our enriched Part B datasets, which has 80+ numerical features and is expected to deliver better evaluation results. (please find the notebook 3.1, 4.1, 5.1 and 6.2 under the supervised-learning folder)
...
/8.2_Model_evaluation_visualization_with_LightGBM.ipynb) under the supervised-learning folder)
